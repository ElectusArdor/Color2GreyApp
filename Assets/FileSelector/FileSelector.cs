...
{	
	...
	{	
		if(open)
		{
			...
			GUI.Window(0, windowDimensions, DrawFileSelector, "Выберите файл " + extension);
		}
	}
	...
	/// </param>
	public static void GetFile(string startingDirectory, SelectFileFunction Callback = null, string extension = ".png")
	{
		if(updater == null) 
		{
	...
	/// 	- The extension of the desired file type.
	/// </param>
	public static void GetFile(SelectFileFunction Callback = null, string extension = ".png")
	{
	...
	private void DrawFileSelector(int ID)
	{
		...
		//Path Buttons
		GUILayout.Label("Путь : ", titleStyle);
		...
		//Directory Buttons
		GUILayout.Label("Папки : ", titleStyle);
		...
		//File Buttons
		GUILayout.Label("Файлы : ", titleStyle);
		...
		GUILayout.BeginHorizontal();
			...
			GUILayout.Label("Выбранный файл : ", titleStyle, GUILayout.Width(titleStyle.CalcSize(new GUIContent("Выбранный файл : ")).x));
